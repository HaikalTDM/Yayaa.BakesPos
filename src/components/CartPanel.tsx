...
export default function CartPanel({ onProceed, onBack }: Props) {
  ...
  if (cart.length === 0) {
    return (
      <div className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t-2 border-brand-pink/20 rounded-t-3xl shadow-[0_-4px_20px_rgba(232,87,122,0.2)] px-5 pt-5 pb-8">
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t-2 border-brand-pink/20 rounded-t-3xl shadow-[0_-4px_20px_rgba(232,87,122,0.2)] transition-all duration-200">
      {!expanded ? (
        <button
            ...
            {cart.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 ml-2">
                  <button
                    onClick={() =>
                      dispatch({ type: 'DECREMENT', product_id: item.product_id })
                    }
                  ...
                  <button
                    onClick={() =>
                      dispatch({ type: 'INCREMENT', product_id: item.product_id })
                    }
